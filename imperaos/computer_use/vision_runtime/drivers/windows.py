...
def screen_capture_provider_factory(
    ...
    return FailClosedScreenCaptureProvider(
        reason_code="WINDOWS_COMPUTER_USE_NOT_QUALIFIED",
        message="Windows capture is scaffolded but not qualified for live computer-use.",
    )
...
def input_executor_factory(config: ComputerUseRuntimeConfig) -> FailClosedInputExecutor:
    ...
    return FailClosedInputExecutor(
        reason_code="WINDOWS_COMPUTER_USE_NOT_QUALIFIED",
        message="Windows input is scaffolded but not qualified for live computer-use.",
    )
